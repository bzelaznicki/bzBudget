...
async function IncomeCard() {
  const summary = await loadSummary()
  const currencyCode = summary.currency?.isoCode ?? "USD"
  const change = percentChange(
    summary.income.current,
    summary.income.previous,
  )

  return (
    <SummaryCard
      title="Income this month"
      value={formatCurrencyValue(summary.income.current, currencyCode)}
      change={change}
...
async function ExpensesCard() {
  ...
  const change = percentChange(
    ...
    summary.expenses.previous,
  )

  return (
    <SummaryCard
      title="Expenses this month"
      value={formatCurrencyValue(summary.expenses.current, currencyCode)}
      change={change}
