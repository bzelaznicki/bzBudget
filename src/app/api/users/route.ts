...
export async function POST(req: Request) {
	try {
		...
		if (!res.lastName || !res.firstName || !res.email) return respondWithError(400, "All fields must be present");

		const user = await createUser(res.firstName, res.lastName, res.email);

		if (!user) return respondWithError(500, "User not created", `Email ${res.email}, ${res.firstName} ${res.lastName} not created`);
